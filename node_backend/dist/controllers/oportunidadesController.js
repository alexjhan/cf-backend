...
export async function crear(req, res, next) {
    try {
        const parsed = OportunidadCreateSchema.parse(req.body);
        const created = await supa.insertOne(TABLE, mapOportunidadToDb(parsed));
        res.status(201).json(mapDbToOportunidad(created));
    }
    catch (e) {
        next(e);
    }
...
export async function actualizar(req, res, next) {
    try {
        const parsed = OportunidadUpdateSchema.parse(req.body);
        const updated = await supa.updateOne(TABLE, req.params.id, mapOportunidadToDb(parsed));
        res.json(mapDbToOportunidad(updated));
    }
    catch (e) {
        next(e);
    }
